...
// On the Nuxt server (SSR / server/api routes) the private NUXT_BACKEND_API_URL
// env var is used so the container can reach the backend via the Docker network.
// In the browser the public NUXT_PUBLIC_BACKEND_API_URL is used so the user's
// browser can reach the exposed port. Both fall back to the Azure-hosted URL.
const getApiBaseUrl = (): string => {
  try {
    const config = useRuntimeConfig()
    const serverUrl = import.meta.server ? (config.backendApiUrl as string) : ''
    return serverUrl || (config.public.backendApiUrl as string) || 'https://ai-investigation-server.azurewebsites.net/api/v1'
  } catch {
    return 'https://ai-investigation-server.azurewebsites.net/api/v1'
  }
}
...
export const caseService = {
  async getCases(): Promise<FraudCase[]> {
    try {
      const response = await $fetch<ApiResponse<FraudCase[]>>(`${getApiBaseUrl()}/cases`, {
        method: 'GET'
      })
  ...
  async getCaseById(id: string): Promise<FraudCase> {
    try {
      const response = await $fetch<ApiResponse<FraudCase>>(`${getApiBaseUrl()}/cases/${id}`, {
        method: 'GET'
      })
      return response.data
    } catch (error) {
  ...
  async analyzeCase(id: string): Promise<void> {
    try {
      await $fetch(`${getApiBaseUrl()}/cases/${id}/analysis`, {
        method: 'POST'
      })
    } catch (error) {
  ...
  async deleteCase(id: string): Promise<void> {
    try {
      await $fetch(`${getApiBaseUrl()}/cases/${id}`, {
        method: 'DELETE'
      })
    } catch (error) {
  ...
  async addNote(id: string, content: string): Promise<any> {
    try {
      const response = await $fetch<ApiResponse<any>>(`${getApiBaseUrl()}/cases/${id}/notes`, {
        method: 'POST',
        body: { content }
      })
      return response.data
  ...
  async deleteNote(caseId: string, noteId: string): Promise<void> {
    try {
      await $fetch(`${getApiBaseUrl()}/cases/${caseId}/notes/${noteId}`, {
        method: 'DELETE'
      })
    } catch (error) {
